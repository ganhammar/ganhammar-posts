...
The response has the shape of an OpenAI completion, with the candidates under `choices[0].logprobs.top_logprobs[0]`, one entry per token text. Each option is then matched to the letter it was presented as, and a letter that did not make the top twenty gets negative infinity, which is what "no probability" looks like in log space:
...
Last, the log probabilities become probabilities. Before normalising, the code sums how much of the model's probability landed on the declared letters at all, which is a number I will come back to. A softmax then runs over the letters that were present, and absent letters stay at zero:
...
`SoftmaxAllowMissing` is a plain softmax with two edge cases: no letters present gives all zeros, and one letter present gives that option probability one. That is the whole readout; the rest of the file is validation and error messages.
