...
On Bedrock the readout is one method, and it reads best in four steps. First the request. The prompt is rendered to a string (more on that below), and the model is asked for one token and the twenty most likely candidates at that position:
...
var responseBytes = await invoke(modelArn, requestBody, cancellationToken);
```

The response has the shape of an OpenAI completion, and the candidates live under `choices[0].logprobs.top_logprobs[0]`, one entry per token text. The second step pulls them out:

```csharp
var choice = payload.GetProperty("choices")[0];
var logprobs = choice.GetProperty("logprobs");
var positions = logprobs.GetProperty("top_logprobs");   // one object per sampled position
...
Third, each option is matched to the letter it was presented as. Option one was `A`, option two was `B`, and so on. A letter that did not make the top twenty gets negative infinity, which is what "no probability" looks like in log space:
...
foreach (var option in options.EnumerateArray())
{
    ...
    optionLogprobs.Add(byLetter.TryGetValue(letters[index].ToString(), out var logprob)
        ? logprob
...
Last, the log probabilities become probabilities. Before normalising, the code adds up how much of the model's probability landed on the declared letters at all, which is a number I will come back to. Then a softmax runs over the letters that were present, and absent letters stay at zero:
...
That is the whole readout. The rest of the file is validation and error messages.
