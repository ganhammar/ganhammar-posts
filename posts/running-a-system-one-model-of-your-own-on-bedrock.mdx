...
I wanted to know how much of that you can get from an open model that you run yourself, on your own AWS account and in your own region, and what it costs you. This post is the answer, with the measurements that go with it. To have something real to decide about, it also builds a pipeline that reads every error log in an AWS account, decides whether each error is ours, and files a GitHub issue with a root cause when it is.
...
The response carries the twenty most likely tokens at the first position, with their log probabilities. The code looks up each option's letter, treats any letter that did not make the top twenty as having no probability, and applies a softmax. That is the whole readout, about forty lines of C#.
