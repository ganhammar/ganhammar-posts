...
In [part 2](https://www.ganhammar.se/posts/blazingly-fast-serverless-note-app-part-2), we'll build the Blazor WebAssembly frontend with MudBlazor for Material Design components, implement OAuth2 authentication, and deploy everything to AWS with CDK. The complete code is available in the [companion repository](https://github.com/ganhammar/smart-note-organizer).
