...
The frontend is plain HTML and JavaScript, served from S3 through CloudFront, where users sign in through the Cognito Hosted UI. The agent runs in a container on AgentCore Runtime, a Bedrock Converse loop with no framework. The MCP server runs on Lambda, compiled with Native AOT, owns the expense table, and enforces every permission. Cognito plays both the identity provider and the downstream authorization server, which keeps the demo self-contained. In a real system the downstream could be any OAuth2 provider. CloudFront also proxies the agent and session binding calls, so the browser talks to a single origin and needs no AWS SDK.

Everything except the frontend is C#, including the CDK stack. There is no AgentCore SDK for .NET, and it turns out you do not need one. The identity APIs ship in the regular AWS SDK, and the runtime contract is two HTTP endpoints.
...
Unauthenticated requests get a 401 with a `WWW-Authenticate` header pointing at the metadata endpoint, which is how spec-compliant MCP clients discover the authorization server. Lambda function URLs rename that header to `x-amzn-remapped-www-authenticate`, though. Our agent gets its tokens from AgentCore Identity instead, so nothing in this system reads the challenge, but an external MCP client pointed straight at the server will trip on it.
...
A thrown `McpException` reaches the model as a tool error, which is what lets the agent relay a refusal in plain language instead of hallucinating around it. The server runs as a Native AOT binary on `provided.al2023` behind a function URL, and the whole stack compiles with zero trim warnings, MCP SDK and JwtBearer included. The build pipeline is in the repository.
...
The broker is one call to AgentCore Identity. Either the vault has a token for this user and returns it, or it returns an authorization URL for the consent flow:
        ...
        string workloadToken)
    {
        var response = await _client.GetResourceOauth2TokenAsync(
            new GetResourceOauth2TokenRequest
            {
                ...
                ResourceCredentialProviderName = "cognito-expenses",
                Scopes = Scopes,
...
The unconsented path is worth a design note. Returning the authorization URL for every message means a user who says "hi" gets an authorization ceremony. Instead, `RunWithoutAccess` runs the model with no tools and puts the link in the system prompt, with instructions to surface it only when the request actually needs expense access. The model decides relevance. As a practical footnote, Nova models interleave `<thinking>` blocks into text output, so the agent strips those before returning anything.
...
The AgentCore constructs recently graduated into stable `aws-cdk-lib`, and the identity pieces are small. Inbound auth on the runtime is one line, and the credential provider is a real CloudFormation resource that takes the agent client's credentials and the user pool's endpoints:
...
The binaries are built as Native AOT on GitHub's arm64 runners, with no containers in the Lambda path and the link step pinned to the glibc that `provided.al2023` ships. That pipeline is a story of its own, and the workflow in the repository is the reference for now.

## The Payoff

Alice asks the agent to submit a coffee expense. The first time, she gets the consent moment, grants access, and the expense appears. Then she tries her luck:

{/* verify wording against the live app before publishing */}

> **alice:** Approve the coffee expense
>
> **agent:** I can't do that. Approving expenses requires being in the managers group, and you are not a member.
...
> **bob:** Approve the coffee expense
>
> **agent:** Done. The coffee expense (45 SEK) submitted by alice is now approved.

The expense item in DynamoDB now reads `approvedBy: bob`. Not `approvedBy: agent`, not a service account, a person. Put the two tokens side by side. Alice's login token carries `openid profile` for the frontend client. The token the MCP server saw carries the expense scopes for the agent client, with the same `sub`. Same human, different power, and the difference is exactly what she consented to.

{/* IMAGE: decoded token pair, login token vs delegated token */}
...
Some rough edges worth knowing about. Cognito auto-grants scopes for its own app clients, so this demo has no real consent screen, just a redirect bounce. With a third party provider like Google or GitHub the user would see a proper one. First use per user costs a full authorization round trip, though vault hits after that are cheap, and this design runs the MCP handshake on every invocation, which production code would cache. Deploys cause a brief gap too, since invocations answer 424 while an endpoint rolls to a new version. And the developer experience is Python first. The decorators and `agentcore dev` conveniences stop there, so C# gets the raw APIs. For this post that was fine, but the gap is real.
...
The pattern generalizes past expenses. Anywhere an agent acts for signed-in users, against any API that speaks OAuth2, the same three tokens and the same binding step apply. I have a smart home full of devices with rather different opinions about who may unlock the front door, and a [Home Assistant MCP server](https://github.com/ganhammar/hass-mcp-server) that suddenly looks like it deserves the same treatment. Perhaps a follow-up.
