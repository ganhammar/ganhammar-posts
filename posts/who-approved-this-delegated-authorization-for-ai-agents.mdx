...
canonical: https://blog.elva-group.com/who-approved-this-delegated-authorization-for-ai-agents-with-agentcore-identity
date: 2026-08-14
status: published
---
...
In 2019, a misconfigured firewall at Capital One was tricked into [lending out its AWS credentials](https://krebsonsecurity.com/2019/07/capital-one-data-theft-impacts-106m-people/). The trick matters less than what those credentials could do: read about a hundred million credit applications out of S3, something no firewall ever needs to do. Three years earlier, an AWS key committed to one of Uber's private repos [unlocked archives on 57 million riders and drivers](https://www.ftc.gov/news-events/news/press-releases/2018/04/uber-agrees-expanded-settlement-ftc-related-privacy-security-claims). That is an old pattern in application security. Software holding more power than any person it serves, with logs that record what the application did, not who asked for it. Before OAuth, integrations worked that way by default. Either the application ran in god mode on a privileged service account, or it asked users for their actual passwords, like RockYou, which collected webmail passwords for contact importing and [leaked 32 million credentials from plaintext storage](https://techcrunch.com/2009/12/14/rockyou-hack-security-myspace-facebook-passwords/) in 2009.

We spent the better part of two decades fixing that. Authorization code flows, scoped access tokens, refresh tokens, revocation. Later, the token exchange standard ([RFC 8693](https://datatracker.ietf.org/doc/html/rfc8693)) gave middle services a way to trade a user's token for a narrower downstream token, what Microsoft calls the on-behalf-of flow. I have [explored this before](https://www.ganhammar.se/posts/delegation-tokens-with-amazon-cognito) in the context of Amazon Cognito, where a service constrains what a downstream actor may do with a user's identity.

Then AI agents arrived, and we regressed about twenty years. The typical agent integration today is an API key in an environment variable, which is god mode all over again. We already know how that story ends. In 2021, the [Codecov breach](https://about.codecov.io/security-update/) harvested exactly these keys out of thousands of CI (continuous integration) environments and used them to reach downstream systems. The agent can do everything its key allows, for every user it serves, and when someone asks "who approved this expense?" the honest answer is "the agent did". That answer is useless. The user the agent acted for is gone from the trail, and revoking one user's access means rotating a key that everyone shares.

Agents need the on-behalf-of flow more than classic services ever did, because an agent is a far less predictable actor than a REST endpoint.

This post builds that flow on AWS, with an agent on Amazon Bedrock AgentCore Runtime that acts on behalf of the signed-in user, holds only the permissions that user granted it, and leaves the user's name in the audit trail, in C# end to end.
...
The frontend is plain HTML and JavaScript, served from S3 through CloudFront, where users sign in through the Cognito Hosted UI. The agent runs in a container on AgentCore Runtime, a Bedrock Converse loop. MCP, the Model Context Protocol, is how agents speak to their tools. The MCP server runs on Lambda, compiled with Native AOT (ahead-of-time compilation), owns the expense table, and enforces every permission. Cognito plays both the identity provider and the downstream authorization server, which keeps the demo self-contained. In a real system the downstream could be any OAuth2 provider. CloudFront also proxies the agent and consent completion calls, so the browser talks to a single origin.

Everything except the frontend is C#, including the CDK (Cloud Development Kit) stack. AgentCore has no high-level SDK for .NET, so the code calls the service APIs from the AWS SDK directly.

![Architecture diagram](./assets/who-approved-this-architecture.png)
...
The first is the login token. Alice signs in to the frontend through the Cognito Hosted UI using the authorization code flow with PKCE (Proof Key for Code Exchange), and her browser ends up with an access token issued to the frontend app client. That client is allowed the scopes `openid` and `profile`, nothing else, so her own token cannot call the expense API.
...
> JWT (JSON Web Token) authorizer configured on the runtime, the
> runtime calls it for you and your code only ever sees the result.
...
One distinction matters in the expense server. Scopes describe what a client may request on a user's behalf, and Cognito grants them per client, not per user. Whether alice may actually approve expenses lives in her `cognito:groups` claim, and that is a property of alice. The approve tool therefore requires both the approve scope on the token and the managers group on the user behind it. Delegation controls how far the agent may reach. The group controls how far the human could reach in the first place.
...
The server is where delegation gets enforced, and it authenticates every request by the token it carries, our own agent included. The [official MCP C# SDK](https://github.com/modelcontextprotocol/csharp-sdk) plugs into standard ASP.NET Core authentication, so the wiring is ordinary JwtBearer against the user pool, plus the MCP challenge scheme:
...
The identity code amounts to an invocation handler and a broker. The handler picks up the workload token the runtime delivered and asks the broker for alice's delegated token:
...
With a token in hand, the agent connects to the MCP server as alice. The delegated token rides the connection, so every tool call downstream carries the caller's identity:
...
Returning the authorization URL for every message would mean a user who says "hi" gets an authorization ceremony. Instead, `RunWithoutAccess` runs the model with no tools and puts the link in the system prompt, with instructions to surface it only when the request actually needs expense access.

> [!NOTE]
> Nova models interleave `<thinking>` blocks into their text output.
> The agent strips those before returning a reply.
...
![Session binding sequence](./assets/who-approved-this-binding.png)
...
- The runtime requires the image to be a single OCI (Open Container Initiative) manifest. Classic `docker build` pushes a Docker v2 manifest that passes ECR, CloudFormation, and the control plane, then fails at session start with a 424 and empty logs. Build with `docker buildx --provenance=false --sbom=false`.
...
The expense item in DynamoDB now reads `approvedBy: bob`. Not `approvedBy: agent`, not a service account, a person. Put bob's two tokens side by side. His login token carries `openid profile` for the frontend client. The token the MCP server saw carries the expense scopes and his group for the agent client, with the same `sub`.
...
The incidents from the introduction make a fair test of what this design changes. The Capital One failure was a component whose role could read data it had no business reading. Here, the runtime's execution role can invoke Bedrock, read one provider secret, and broker tokens. It has no access to the expense table, so stealing the agent's own credentials yields no expenses. The Uber failure was a static key that worked for whoever held it. The equivalent artifact here is the agent client secret, and it only participates in the authorization code flow, so a leaked copy produces nothing without a signed-in user consenting. Codecov is the one this design does not answer. The deploy pipeline's role can change the MCP server itself, and delegation downstream does not help once the pipeline is compromised.

A compromised agent is a different case from a leaked credential. An attacker with a foothold in the runtime, or a prompt injection carried in a poisoned expense description, acts with the delegated token of whoever is talking to the agent. An API key gave away every user at once, offline and indefinitely. This reaches a single user's data, inside that user's grant, only while that user is active, and disabling the user ends it within the access token lifetime. What remains exposed is whatever the model can be talked into doing with that authority.

That leads to an accountability wrinkle. If bob's agent approves an expense because the expense description talked it into it, the trail still reads `approvedBy: bob`. Delegation settles whose authority the agent used, and leaves open who made the decision. For actions that matter, that argues for a confirmation step in front of the human.

The token vault concentrates live tokens for every user of the agent, gated behind the workload identity and encrypted with a service managed key, with a customer managed key as an option. A security review should still treat it as a credential store.

## Revoking a Grant

The grant lives in the vault as a refresh token. Each renewal asks Cognito for a fresh access token, which arrives with the claims Cognito holds at that moment, and the refresh token itself stays valid for as long as the agent client allows, thirty days on default settings. A grant that nobody touches persists across sessions for that long. This is the same persistence the earlier note on widened scopes ran into.

Ending it runs through Cognito. Offboarding means disabling the user, which stops the vault from renewing the grant:
...
What remains after that is the access token already issued, which the MCP server validates statelessly and accepts until it expires, up to an hour on default settings. The access token lifetime is a setting on the agent app client if an hour is too long. [Global sign-out](https://docs.aws.amazon.com/cognito/latest/developerguide/token-revocation.html) revokes the refresh tokens too, but for a user who is still enabled it buys less than it appears to. The next invocation gets a fresh consent link, and one click restores the grant, so on its own it is a session reset. Removing bob from the managers group is the role downgrade. It takes effect within the access token lifetime, because the next renewal mints a token with the current group claims.

The AgentCore API has no operation that removes a single entry from the vault. The levers are the user, the group, and, for everything at once, deleting the credential provider. A shared API key gives you one lever, rotating it for everyone.
...
The full example is in the [GitHub repository](https://github.com/ganhammar/who-approved-this): agent, MCP server, CDK stack, frontend, and the deploy pipeline. To run it yourself, fork it, point the workflow at an OIDC (OpenID Connect) deploy role, set a demo password secret, and push. The README covers the details.
