...
> [!TIP]
> The fastest way to find out whether AOT is realistic for an existing
> project is to publish with `-p:PublishAot=true` and read the trim
> warnings. They name the reflection your dependencies rely on before
...
> [!NOTE]
> This is the only reason to stay on x64. Building on an ARM64 runner
> lets you target `linux-arm64` and switch to `Architecture.ARM_64`,
> which is cheaper per invocation.
