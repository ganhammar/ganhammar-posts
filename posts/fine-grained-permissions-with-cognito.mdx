...
We will also add a get-booking Lambda handler, which we will use as a proof-of-concept to verify that the permissions is present and can be used to determine wether or not the user is authorized to read bookings. We'll add the handler, `get-booking.ts`, in the `handlers`-folder that we created before.
